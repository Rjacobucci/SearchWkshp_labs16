...
### Linear Regression
```{r}
data.1 = read.table("C:/Users/RJacobucci/Documents/GitHub/SearchWkshp_labs16/apexpos.dat")
...
```{r,message=FALSE}
library(data.table)
progdata <- fread("http://www.stat.nus.edu.sg/~zhangjt/books/Chapman/datasets/rawdata/prog.txt")

names(progdata) = c("group","subject","cycle","day","logp","missing")

# Delete missing data
progdata = progdata[progdata$missing==0,]
```
...
library(ggplot2)
ggplot(data=progdata, aes(x=day,y=logp,group=cycle)) + geom_point() + geom_line() +
...
ggplot(data=data1,aes(x=day,y=logp)) + geom_point(size=3) + geom_line(size=1) +
	geom_line(data=newdata,aes(x=day,y=fity),color="red",size=1) + ggtitle(paste('Polynomial of degree ',degree)) +
...
time = seq(myrange[1],myrange[2],by=0.01) # time points to evaluate the fitted model
# time points and polynomial evaluated at the time points
...
ggplot(data=data1,aes(x=day,y=logp)) + geom_point(size=3) +
	geom_line(size=1) + geom_line(data=newdata,aes(x=time,y=fity),color="red",size=1) +
